...
window.timerRunning = window.timerRunning || false;
...
function saveState() {
  const state = {
    ...
    timer: window.timer,
    timerRunning: window.timerRunning,
...
function loadState() {
  ...
  if (savedState) {
    ...
    window.timerRunning = state.timerRunning || false;
...
function startTimer() {
  if (window.timerInterval) clearInterval(window.timerInterval); // Limpa intervalo anterior
...
function shuffleArray(array) {
  for (let i = array.length - 1; i > 0; i--) {
    ...
    [array[i], array[j]] = [array[j], array[i]]; // Correção da sintaxe
  }
  return array;
}
